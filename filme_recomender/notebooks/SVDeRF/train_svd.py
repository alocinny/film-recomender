...
def train_random_forest(X_train, y_train):
    ...
    # Nota: Não fazemos split aqui dentro. Usamos 100% dos dados para treino.
    
    regr = RandomForestRegressor(n_estimators=100, n_jobs=-1, random_state=42)
    regr.fit(X_train, y_train)

    print("Modelo Híbrido treinado (sem teste).")
...
def run():
    ...
    # Agora usamos 100% do dataset como treino
    train_df = df  
    ...
    # 4. DATASET HÍBRIDO
    print("Criando dataset híbrido COMPLETO...")
    ...
    regr = train_random_forest(X_train, y_train)
